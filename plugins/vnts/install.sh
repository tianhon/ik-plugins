...
install()
{
	...
	# 4、boot:开机启动
	type=$1 
	rm -f /tmp/iktmp/plugins/vnts_installed
	...
	if [ ! -f $INSTALL_DIR/bin/vnts ];then
		mkdir -p $INSTALL_DIR/bin
		tar -xzf $INSTALL_DIR/bin/vnts.tar.gz -C $INSTALL_DIR/bin
		rm $INSTALL_DIR/bin/vnts.tar.gz
		chmod +x $INSTALL_DIR/bin/vnts
	fi

	# 自动启动插件
	[ -f "$EXT_PLUGIN_CONFIG_DIR/$PLUGIN_NAME/autostart" ] && /usr/ikuai/function/plugin_$PLUGIN_NAME start

	touch /tmp/iktmp/plugins/vnts_installed
}
